...
import SpecialiteList from "./lists/SpecialiteList";
  ...
  const handleSubmit = async (event) => {
    ...
    if (buttonRef.current.innerText === "AJOUTER") {
      const response = await axios.post(
        "http://localhost:8080/specialites/save",
        {
          nom: data.get("ville"),
        }
      );
  ...
  return (
    <Context.Provider value={[modify, setModify]}>
      <Box>
        ...
        <SpecialiteList data={data}></SpecialiteList>
      </Box>
    </Context.Provider>
  );
}
