...
export default function ZoneList(props) {
  ...
  const handledelete = async (id) => {
    const response = await axios.delete(
      `http://localhost:8080/zones/delete/${id}`
    );
          ...
          <TableBody>
            {props.data.map((data) => (
              <TableRow key={data.id}>
                ...
                <TableCell>{data.ville.nom}</TableCell>
                <TableCell>
                  <AutoFixNormalIcon
                    className="blue"
                    onClick={() =>
                      setModify({
                        id: data.id,
                        nom: data.nom,
                        ville: { id: data.ville.id, nom: data.ville.nom },
                      })
                    }
                  />
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
